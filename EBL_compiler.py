...
def add_decorator_command(
    ...
    if type(event_cls) in eternalevents.SPAWN_TARGET_EVENTS:
        ...
        if type(event_cls) == eternalevents.SpawnSingleAI:
            spawn_type = event_cls.spawnType
        elif type(event_cls) == eternalevents.SpawnArchvile:
            spawn_type = "ARCHVILE"
        elif type(event_cls) == eternalevents.SpawnPossessedAI:
            spawn_type = "SPIRIT"
        new_entity_args = [
            f"mod_autogenerated_spawn_target_{mod_target_idx}",
            spawn_type,
        ]
        print(f"{spawn_type=}")
        mod_target_idx += 1
        event_cls.spawnTarget = new_entity_args[0]
        modified_args = get_event_args(event_cls)
    else:
        ui_log(f"WARNING: event {type(event_cls)} has no associated tags")
        return modified_args

    decorator_changes += [(entity_name, new_entity_args, cmd, event_cls)]
    return modified_args


def apply_decorator_command(
    entity: str,
    new_entity_args: list,
    cmds: str,
    event_cls: eternalevents.EternalEvent,
) -> Tuple[str, bool]:
    # ui_log(f"Applying '{cmd}' to '{entity}' with class '{type(event_cls)}'")
    do_not_modify = False

    print(entity)
    parsed_entity = parser.ev.parse(entity)
    entitydef = ""
    ...
    print(f"Applying '{cmds}' to '{entitydef}' with class '{type(event_cls)}'")
    cmd_list = [cmd.strip() for cmd in cmds.split(";")]
    for cmd in cmd_list:
        cmd_name, args = cmd.split(" ", 1)
        if cmd_name == "anim":
            animweb = "animweb/characters/monsters/zombie_tier_1/traversal/" + args[0]
            try:
                ...
                parsed_entity[f"entityDef {new_entity_args[0]}"] = parsed_entity.pop(
                    entitydef
                )
            except KeyError:
                ui_log("ERROR: couldn't access fields")
            print(parser.generate_entity(parsed_entity))
            ...
            try:
                ...
                parsed_entity[entitydef]["edit"]["entityDefs"] = {
                    "num": 1,
                    "item[0]": {"name": new_entity_args[1]},
                }
                parsed_entity[entitydef]["edit"]["targets"] = {
                    "num": 1,
                    "item[0]": new_entity_args[1],
                }
                parsed_entity[f"entityDef {new_entity_args[0]}"] = parsed_entity.pop(
                    entitydef
                )
            ...
            ui_log(
                f"WARNING: Tag {cmd_name} is not compatible with event {type(event_cls)}"
            )

        return parser.generate_entity(parsed_entity), do_not_modify
...
def apply_ebl(
    ...
    for key, val in deltas:
        if val[0] == "TEMPLATE":
            ...
            t_name, t_args = parse_event(key)  # key.split("(", 1)
    ...
    with open(modded_file, "w") as fp:
        ...
        for entity in entities:
            total_count += 1
            for decorated_entity, new_entity_args, cmd, event_cls in decorator_changes:
                if f"entityDef {decorated_entity} {{" in entity:
                    new_entity, do_not_modify = apply_decorator_command(
                        entity, new_entity_args, cmd, event_cls
                    )
                    if do_not_modify:
                        fp.write(new_entity)
                    else:
